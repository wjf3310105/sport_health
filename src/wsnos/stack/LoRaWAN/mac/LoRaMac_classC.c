...
static stack_callback_t classC_callbacks;

static uint32_t lora_classc_send_seq = 0;
static mbuf_t *l_mbuf = NULL;

PROCESS_NAME(loramac_classc_init_process);
PROCESS_NAME(loramac_classc_join_process);
PROCESS_NAME(loramac_classc_send_process);
...
static void McpsConfirm( McpsConfirm_t *mcpsConfirm )
{
    if (mcpsConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK)
    {
        ...
        case MCPS_UNCONFIRMED:
            // check datareate
            // check txpower
            if (classC_callbacks.tx_confirm != NULL)
                classC_callbacks.tx_confirm(TRUE, l_mbuf->seq, l_mbuf->head, l_mbuf->data_len);
            break;
        ...
        case MCPS_CONFIRMED:
            ...
            if (classC_callbacks.tx_confirm != NULL)
                ...
                {
                    classC_callbacks.tx_confirm(FALSE, l_mbuf->seq, l_mbuf->head, l_mbuf->data_len);
                }
            break;
...
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    if ( mcpsIndication->RxData == true )
    {
        ...
        case 2:
            if (classC_callbacks.rx_indicate != NULL)
                classC_callbacks.rx_indicate(mcpsIndication->Buffer,
...
static void MlmeConfirm( MlmeConfirm_t *mlmeConfirm )
{
    ...
    case MLME_JOIN:
    {
        if ( mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK )
        {
            ...
            NextTx = true;
            if (classC_callbacks.join_confirm != NULL)
                classC_callbacks.join_confirm(TRUE);
            break;
        }
        else
        {
            ...
            event.param = NULL;
            osel_post(NULL, &loramac_classc_join_process, &event);
        }
        break;
    }
...
static void lorawan_state_init_handle(void)
{
    ...
    osel_pthread_create(mac_task_tcb, &loramac_classc_join_process, PROCESS_CURRENT());
}
...
static void lorawan_state_join_handle(void)
{
    ...
    osel_pthread_create(mac_task_tcb, &loramac_classc_send_process, PROCESS_CURRENT());
#endif
}
...
PROCESS(loramac_classc_init_process, "loRaMAC classC init thread");
PROCESS_THREAD(loramac_classc_init_process, ev, data)
{
    PROCESS_BEGIN();
...
PROCESS(loramac_classc_join_process, "loRaMAC classC join thread");
PROCESS_THREAD(loramac_classc_join_process, ev, data)
{
    PROCESS_BEGIN();
...
PROCESS(loramac_classc_send_process, "loRaMAC classC send thread");
PROCESS_THREAD(loramac_classc_send_process, ev, data)
{
    PROCESS_BEGIN();
...
bool_t mac_init(void *mac_pib)
{
    ...
    classC_callbacks.tx_confirm   = NULL;
    classC_callbacks.rx_indicate  = NULL;
    classC_callbacks.join_confirm = NULL;
...
{
    osel_pthread_create(mac_task_tcb, &loramac_classc_init_process, NULL);
}


void mac_class_stop(void)
{
    osel_pthread_exit(mac_task_tcb, &loramac_classc_init_process, PROCESS_CURRENT());
    osel_pthread_exit(mac_task_tcb, &loramac_classc_join_process, PROCESS_CURRENT());
    osel_pthread_exit(mac_task_tcb, &loramac_classc_send_process, PROCESS_CURRENT());
}
...
void mac_class_dependent_cfg(const stack_callback_t *callbacks)
{
    ...
    classC_callbacks.tx_confirm   = callbacks->tx_confirm;
    classC_callbacks.rx_indicate  = callbacks->rx_indicate;
    classC_callbacks.join_confirm = callbacks->join_confirm;
}
...
int8_t mac_class_send(msg_flag_t flag, mbuf_t *mbuf)
{
    ...
    l_mbuf = mbuf;
    l_mbuf->seq  = lora_classc_send_seq++;
    l_mbuf->flag = flag;
    ...
    event.sig = PROCESS_EVENT_MSG;
    event.param = mbuf;
    osel_post(NULL, &loramac_classc_send_process, &event);
