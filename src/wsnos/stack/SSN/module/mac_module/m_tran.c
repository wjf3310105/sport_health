...
#define  BE_INIT                1
#define  MAX_BE                 3
#define  MAX_CSMA_BACKOFFS      1

#define  PRE_SYNC_CODE_LEN      (6u)

#if DATA_RATE == 250u
/* 250K:32us/B; 4M:2us/B; 50us处理时延 */
#define  LEN_TO_US(len)         (((len+PRE_SYNC_CODE_LEN)*(32ul+2)) + 50)
...
#if DATA_RATE == 100u
#define  LEN_TO_US(len)         (((len+PRE_SYNC_CODE_LEN)*(80ul+2)) + 100u)
...
#if DATA_RATE == 10u
#define  LEN_TO_US(len)         (((len+PRE_SYNC_CODE_LEN)*(1200ul+20)) + 500u)
/* 10K速率，500*(8*4)=1600 增加160的抖动时间（经验值）*/
#define  BE_TO_US(be)           ((be)*30040u)
#define  BE_TO_MS(be)           ((be)*30u)
#define  MS_FOR_ACK             (20u)
#endif

#if DATA_RATE == 1u
#define  CEIL(x)                ((x+1)/4)
#define  LEN_TO_US(len)         ((((CEIL(len)*5+8)+(PRE_SYNC_CODE_LEN+4))*(2100ul+20)) + 5000ul)
/* 10K速率，500*(8*4)=1600 增加160的抖动时间（经验值）*/
#define  BE_TO_US(be)           ((be)*130040ul)
#define  BE_TO_MS(be)           ((be)*130u)
#define  MS_FOR_ACK             (85u)
#endif
...
#define TX_ERROR_RESUME_TIME            (1u)
...
static uint8_t cca_detect_times = 0;    //*< 检测CCA次数
static uint8_t sent_detect_times = 0;   //*< 空中确定发送次数
...
uint8_t m_tran_cca_times(void)
{
    return cca_detect_times;
}

uint8_t m_tran_sent_times(void)
{
    return sent_detect_times;
}
...
static void tx_ok_error_timer(void *arg)
{
    ...
    tran_cb_send_msg(M_TRAN_TX_ERROR_EVENT);
}
#endif
...
static void tran_send(pbuf_t *pbuf)
{
    ...
    if ((pbuf != NULL) && (slot_remain_time > send_need_time))
    {
        ...
        sent_detect_times++;    //*< 空中确认发送次数
#if M_TRAN_DGB_EN > 0
        m_tran_tracker.data_send_real++;
#endif

#if TXOK_INT_SIMU_EN > 0
        HAL_TIMER_SET_REL(US_TO_TICK(LEN_TO_US(pbuf->data_len) + 20000),
                          tx_ok_error_timer,
                          NULL,
                          txok_timer);
        DBG_ASSERT(txok_timer != NULL __DBG_LINE);
...
//    hal_rf_flush_rxfifo();
    ...
    phy_set_state(PHY_RX_STATE);        //PHY_RX_STATE
}
...
{
    if (++simu_int_cnt >= TX_ERROR_RESUME_TIME)
    {
        tran_cb_send_msg(M_TRAN_RESUME_EVENT);
    }
...
//        rf_enter_sleep_force();
    ...
    else //*< 中继实际调试阶段会进入这个分支，但继续往下执行没有问题
    {   DBG_ASSERT(FALSE __DBG_LINE);   }
#endif
}

void tran_deal_resume_event(void)
{
    // static uint32_t resume_event = 0;
    // resume_event++;
    m_tran_sleep();     //*< 先把当前的数据给停掉
    //*< 当前radio处于sleep态的话，POR无法把radio从低功耗状态reset，需要先切换到idle
    m_tran_stop();
    //*< init radio register
    uint8_t rf_channel = 0;
    uint8_t rf_power = 0;
    SSN_RADIO.get_value(RF_CHANNEL, &rf_channel);
    SSN_RADIO.get_value(RF_POWER, &rf_power);
    SSN_RADIO.init();

    SSN_RADIO.set_value(RF_CHANNEL, rf_channel);
    SSN_RADIO.set_value(RF_POWER, rf_power);

    //*< init radio interrupt
...
PROCESS_THREAD(tran_event_process, ev, data)
{
    ...
    while (1)
    {
        ...
        else if (ev == M_TRAN_RESUME_EVENT)
        {
            tran_deal_resume_event();
        }
#endif
