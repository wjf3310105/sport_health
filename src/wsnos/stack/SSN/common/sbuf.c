...
#if SBUF_DBG_EN > 0
        DBG_LOG(DBG_LEVEL_TRACE, "alloc-line:%d, unused:%d, used:%d\r\n",
                sbuf->alloc_line, sbuf_cnt.free, sbuf_cnt.used);
#endif
    }
...
#if SBUF_DBG_EN > 0
    DBG_LOG(DBG_LEVEL_TRACE, "free-line :%d, unused:%d, used:%d\r\n",
            (*sbuf)->free_line, sbuf_cnt.free, sbuf_cnt.used);
#endif
    *sbuf = NULL;
}
