...
#include "stack/SSN/common/prim.h"
...
DBG_THIS_MODULE("nwk_ed_handles");
...
PROCESS_NAME(nwk_reset_process);
                                 ...
                                 uint8_t len)
{
    ...
    if (sbuf->primtype != N2M_JOIN_RESPONE)
        sbuf->primtype = N2M_DATA_REQUEST;

    //*< if join respone. send data to uplink queue. that will send data immediately.
    if (sbuf->primtype == N2M_JOIN_RESPONE)
                                  ...
                                  nwk_join_req_t *join_req)
{
    ...
    join_req->hop_num     = 0x00;
    join_req->reserved    = 0x00;
...
//    next_hop.mode = MAC_MHR_ADDR_MODE_SHORT;
    next_hop.short_addr = mac_pib_coord_saddr();
    sbuf->up_down_link = IMMEDI_LINK;
...
//    next_hop.mode = MAC_MHR_ADDR_MODE_SHORT;
    ...
    if (route_entry == NULL)
    {
        DBG_LOG(DBG_LEVEL_INFO, "can't find nwk_addr=0x%04x\n", nwk_addr);
        ...
        {
            if (route_entry->next_hop == mac_pib_coord_saddr())
            {
                sbuf->up_down_link = IMMEDI_LINK;
            }
            else
            {
                sbuf->up_down_link = WAIT_LINK;
            }
...
static void nwk_frm_data_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
    ...
    if (nwk.device_type == NODE_TYPE_ROUTER)
    {
        ...
        if ((nwk_hd->src_addr & 0xFF00) == (nwk.nwk_addr & 0xFF00))  //*< 直连终端
        {
            ...
            if (res == FALSE)
            {
                DBG_LOG(DBG_LEVEL_INFO, "don't find nwk_id: %x\n", nwk_hd->src_addr);
                route_entry_t route_entry;
                route_entry.nui      = sbuf->primargs.prim_arg.mac_prim_arg.src_addr;
    ...
    if (nwk_is_point_to_local(nwk_hd))
    {
        ...
        nwk_prim_arg->rssi = pbuf->attri.rssi_dbm;
        sbuf->primtype = N2A_DATA_INDICATION;

        if (nwk.dependent.nwk_data_indicate != NULL)
        {
            nwk.dependent.nwk_data_indicate(sbuf);
        }
        else
        {
            pbuf_free(&(sbuf->primargs.pbuf) __PLINE2);
    ...
    else
    {
        if (nwk.device_type == NODE_TYPE_ROUTER)
        {
            ...
            if (route_entry == NULL)
            {
                father_id = ((uint16_t)nwk_hd->dst_addr & 0xFF00) | 0x0001;
                sbuf->up_down_link = IMMEDI_LINK;
                nwk_frames_forward(sbuf, father_id, father_id);
            }
            else
            {
                if (route_entry->next_hop == mac_pib_coord_saddr())
                ...
                else if (mac_assoc_table_find((assoc_node_t *)(&route_entry->next_hop)))
                {
                    sbuf->up_down_link = WAIT_LINK;
...
{
    if ((nwk.device_type == NODE_TYPE_TAG) || (nwk.join_status != NET_JOIN_OK))
    {
        pbuf_free(&(sbuf->primargs.pbuf)__PLINE2);
        sbuf_free(&sbuf __SLINE2);
        return;
    ...
    //*< TODO change nui to mac-id
    assoc_node_t node;
    node.node_id = 0x00ul;

    id_general((uint8_t *)&nwk_join_req.nui, 8, &(node.node_id));
    if (mac_assoc_table_find(&node))
    {
        if (nwk_hd->src_addr == 0x0000)
        {
...
//*< 只保留中继的路由
    ...
    pbuf->data_p -= len;
    pbuf->data_len = 0;
    len = nwk_frames_join_req_fill(pbuf, &nwk_join_req);
    len += nwk_frames_hd_addr_fill(pbuf, nwk_hd);
    sbuf->up_down_link = IMMEDI_LINK;

    sbuf->primargs.prim_arg.mac_prim_arg.msdu_length = len;
...
static int8_t nwk_frm_join_response_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
    ...
    if (nwk_join_resp.nui == nwk.node_nui)
    {
        ...
        if (nwk_join_resp.join_res == NET_JOIN_OK)
        {
            ...
            mac_assoc_respone_can_send(TRUE);
        ...
        {
            DBG_LOG(DBG_LEVEL_INFO, "nwk_frm_join_response_event FAILED\r\n");
            nwk.join_cnt    = 0;
            nwk.nwk_addr    = 0;
            nwk.hop_num     = 0;
...
static void nwk_frm_route_response_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
    ...
    else
    {
        ...
        {
//            route_entry.nui = (nwk_route_resp.dst_nwk_id& 0xFF00) | 0x0001;
//            route_entry.nwk_addr = nwk_route_resp.dst_nwk_id;
//            route_entry.next_hop = sbuf->primargs.prim_arg.mac_prim_arg.src_addr;
//
//            osel_int8_t s = 0;
...
static int8_t nwk_frm_reset_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
    nwk_reset_req_t reset_req;
    pbuf_t *pbuf = sbuf->primargs.pbuf;
    if (nwk_frames_reset_req_get(pbuf, &reset_req) == 0)
        return -1;
    ...
    if (nwk_is_point_to_local(nwk_hd))
    {
        ...
        if (reset_req.identify == 0xAA55)
        {
            osel_pthread_create(nwk_task_tcb, &nwk_reset_process, NULL);
            return 0;
        }
        else
        {
...
//        break;
    ...
    case NWK_FRM_TYPE_RESET:
        nwk_frm_reset_event(sbuf, nwk_hd);
        break;
...
static void nwk_resend_data_packet(sbuf_t *sbuf)
{
    ...
    // DBG_LOG(DBG_LEVEL_INFO, "nwk data resend: len=%d\r\n", len);
    mac_node_addr_t next_hop;
    next_hop.short_addr = sbuf->primargs.prim_arg.mac_prim_arg.dst_addr;
    // sbuf->up_down_link = IMMEDI_LINK;
    sbuf->primargs.pbuf->data_len = len;
...
static int8_t mac2nwk_data_confirm(sbuf_t *sbuf)
{
    ...
    else
    {
        ...
        {
            ret = mac2nwk_data_confirm_layer_handle(sbuf);
            if (++nwk_sent_cnt >= CONTINUATION_CNT)
            {
                DBG_LOG(DBG_LEVEL_INFO, "nwk lost connected...\r\n");
                if (nwk.dependent.nwk_join_indicate != NULL)
                {
                    nwk.dependent.nwk_join_indicate(FALSE);
                }
                mac_assoc_respone_can_send(FALSE);
                nwk_sent_cnt = 0;
                nwk_stop();
                mac_stop();
                mac_run();
...
PROCESS_THREAD(nwk_join_process, ev, data)
{
    ...
    while (1)
    {
        ...
        {
            DBG_LOG(DBG_LEVEL_INFO, "nwk_join_process set hops: %d\n", mac_pib_hops());
            nwk_join_request_packed();
            OSEL_ETIMER_DELAY(&(nwk.join_etimer), 600 * mac_pib_hops() * 2);
        }
...
PROCESS(nwk_reset_process, "nwk_reset_process");
PROCESS_THREAD(nwk_reset_process, ev, data)
{
    PROCESS_BEGIN();

    static osel_etimer_t reset_etimer;

    OSEL_ETIMER_DELAY(&reset_etimer, 100);
    hal_board_reset();
        ...
        if (!list_empty(&query_head))
        {
            ...
            else if (sbuf->primtype == A2N_DATA_REQUEST)
            {
                ...
                if (route_entry == NULL)
                {
                    ...
                    osel_etimer_arm(&(sbuf->etimer), 300 * mac_pib_hops() * 2, 0);
                    ...
                    else if (ev == NWK_QUERY_RESP_EVENT)
                    {
                        ...
                        osel_etimer_disarm(&(sbuf->etimer));
...
static void nwk_info_config(void)
{
    nwk.live_cnt      = 10;
    nwk.device_type   = nwk_get_type();
    nwk.father_id     = sink_nwk_address;
    nwk.hop_num       = 0;
    nwk.node_nui      = mac_pib_self_saddr();

    nwk.join_status     = NET_JOIN_LIC_ADDR;
    nwk.nwk_addr        = 0x0000;
}

inline uint16_t nwk_father_get(void)
{
    return nwk.father_id;
}
