...
/** NWK qos 占用字节数 */
#define NWK_QOS_SIZE                0

/** NWK seq 占用字节数 */
#define NWK_SEQ_SIZE                0
...
typedef enum {
    ...
    NWK_FRM_TYPE_RESET      = 7,            //*< NWK复位请求帧
    NWK_FRM_TYPE_RESERVED,            //*< NWK保留类型
} nwk_frm_type_enum;
...
typedef struct
{
    ...
    uint16_t dst_addr;                      //*< 目的地址（根据控制域决定是8字节还是2字节）
    uint16_t src_addr;                      //*< 源地址（根据控制域决定是8字节还是2字节）
} nwk_hd_addr_t;
...
uint8_t nwk_frames_reset_req_get(pbuf_t *pbuf, nwk_reset_req_t *req);

uint8_t nwk_frames_reset_req_fill(pbuf_t *pbuf, nwk_reset_req_t *req);
