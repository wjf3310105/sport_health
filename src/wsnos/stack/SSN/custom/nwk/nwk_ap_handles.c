...
DBG_THIS_MODULE("nwk_ap_handles");
...
static int8_t nwk_frames_reset_request(uint16_t dst_addr,
                                       uint32_t dst_mac)
{
	...
	nwk_addr.nwk_hd_ctl.frm_ctrl = NWK_FRM_TYPE_RESET;
	...
	reset_req.identify = 0xAA55;
	uint8_t len2 = nwk_frames_reset_req_fill(pbuf, &reset_req);
	if (len2 == 0)
	{
		pbuf_free(&pbuf __PLINE2);
		sbuf_free(&sbuf __SLINE2);
...
static void nwk_frm_data_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
	...
	if (nwk.device_type == NODE_TYPE_GATEWAY)
	{
		...
		if (res == FALSE)
		{
			DBG_LOG(DBG_LEVEL_INFO, "don't find nwk_id: 0x%04x\n", nwk_hd->src_addr);

			nwk_addr_long_get(nwk_hd->src_addr, &nui);
			if (nui != 0x00ull)
			{
		...
		{
			osel_int8_t s = osel_mutex_lock(OSEL_MAX_PRIO);
			route_entry_t *route_node = nwk_route_module_nexthop(nwk_hd->src_addr);
			route_node->next_hop = sbuf->primargs.prim_arg.mac_prim_arg.src_addr;
			osel_mutex_unlock(s);
		}
	}
	...
	if (nwk_is_point_to_local(nwk_hd))
	{
		...
		nwk_prim_arg->key  = nui;
		sbuf->primtype = N2A_DATA_INDICATION;

		if (nui == 0x00ull)
		{
			nwk_frames_reset_request(nwk_hd->src_addr,
			                         sbuf->primargs.prim_arg.mac_prim_arg.src_addr);
		}
	...
	else
	{
		...
		if (mac_assoc_table_find((assoc_node_t *) & (route_entry->next_hop)))
		{
			sbuf->up_down_link = WAIT_LINK;
...
static void nwk_frm_join_request_event(sbuf_t *sbuf, nwk_hd_addr_t *nwk_hd)
{
...
#if DEBUG_NWK_AUTH_EN

	if (!nwk_addr_join_auth(route_entry.nui))	//*< add join auth.
	{
		PRINTF("NUI: 0x%lx is auth failed\r\n", route_entry.nui);
		return;
	}
#endif

	uint16_t find_saddr = 0x0000;
	if (nwk_addr_short_find(nwk_join_req.nui, &find_saddr))
		nwk_addr_del(find_saddr);

	nwk_join_req.hop_num ++;
	nwk_addr_table_add(nwk_join_req);
	route_entry.nui = nwk_join_req.nui;
	route_entry.nwk_addr = nwk_addr_short_get(&nwk_join_req);
	route_entry.next_hop = sbuf->primargs.prim_arg.mac_prim_arg.src_addr;
	...
	sbuf->primtype = N2M_JOIN_RESPONE;
	sbuf->primargs.prim_arg.mac_prim_arg.dst_addr = route_entry.next_hop;
	sbuf->primargs.prim_arg.mac_prim_arg.src_addr = mac_pib_src_saddr();
...
static int8_t mac2nwk_data_indication(sbuf_t *sbuf)
{
	pbuf_t *pbuf = sbuf->primargs.pbuf;
	DBG_ASSERT(pbuf != NULL __DBG_LINE);
	PRINTF("mac2nwk_data_indication\n");
...
void nwk_run(void)
{
	...
	if (nwk.join_status == NET_JOIN_OK)
	{
		...
		osel_pthread_create(nwk_task_tcb, &nwk_cycle_process, PROCESS_CURRENT());
		return;
	}
