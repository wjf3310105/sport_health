...
#define MS_TO_TICK(time_ms)     ((uint32_t)((time_ms) * 1))
#define US_TO_TICK(time_us)     ((uint32_t)(((time_us) / 1000.0) * 1))
#define TICK_TO_US(tick_num)    ((uint32_t)((tick_num) * 1000))
...
#define HTIMER_SW_INC(x)            st(x++;)
#define HTIMER_HWCOUNT(x)           x=hardware_cnt
