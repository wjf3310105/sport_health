...
volatile uint16_t backvalue_wdt_cnt = 0;
volatile uint16_t data_send_wdt_cnt = 0;
...
void wdt_clear(uint32_t time_ms)
{
    ...
    {
        TA0CCR3 = TA0R + 0x8000;  // 1s
        TA0CCTL3 |= CCIE;
    }
}

void wdt_extern_clear(void)
{
    //P2.3->WDI
    P2SEL &= ~BIT3;
    P2DIR |= BIT3;
...
void wdt_start(uint32_t time_ms)
{
    if (time_ms == 300000ul)
    {
        if (!backvalue_wdt_start)//背景5分钟
        {
            backvalue_wdt_start = TRUE;
            backvalue_wdt_cnt = time_ms/1000;
            TA0CCR1 = TA0R + 0x8000; // 1s
            TA0CCTL1 |= CCIE;
        }        
    }
    else if (time_ms == 1200000ul)//数据发送20分钟
    {
          data_send_wdt_cnt = time_ms/1000;
          TA0CCR2 = TA0R + 0x8000; // 1s
          TA0CCTL2 |= CCIE;            
    }
}
...
{
    if (time_ms == 300000ul) // 5分钟看门狗，用于保护背景值更新错譍
    {
        backvalue_wdt_start = FALSE;
        TA0CCTL1 &= ~CCIE;
    }
    else if (time_ms == 1200000ul)//20分钟看门狗,用于保护数据发送出错
    {
        TA0CCTL2 &= ~CCIE;        
    }
    else
    {
        TBCCTL0 &= ~CCIE;  //由于有周期性定时器存在，TimerB不允许停櫍
    }
}
...
static uint32_t get_system_clock_value(uint8_t system_clock_speed)
{
...
void clk_init(uint8_t system_clock_speed)
{
    ...
    TB0CCR2 = TB0R + 0xc99;     //100ms start extern wdt process
    TB0CCTL2 |= CCIE;
    wdt_extern_clear();

    TA0CCR3 = TA0R + 0x8000;  // 10ms - 0x147, 1s - 0x8000
    TA0CCTL3 |= CCIE;
}
...
__interrupt void timer_b1_ccrx( void )
{
    ...
    case 4:
        wdt_extern_open();
        TB0CCR1 = TB0R + 0x60;      //*< 3MS
        TB0CCTL1 |= CCIE;
        TB0CCR2 = TB0R + 0x8000;    //*< 1s
        break;
