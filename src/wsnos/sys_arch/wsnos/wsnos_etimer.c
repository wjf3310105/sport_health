...
#if OSEL_ETIMER_SOFTWARE
#include "common/hal/hal_timer.h"
#include "platform/platform.h"
PROCESS_NAME(osel_etimer_process);

static uint16_t osel_etimer_tick = 10;
static hal_timer_t *osel_etimer_htimer = NULL;
#endif
...
static void osel_etimer_tick_cb(void *p)
{
    osel_etimer_htimer = NULL;

    osel_event_t event;
    event.sig = PROCESS_EVENT_TIMER;
    event.param = NULL;
    osel_post(NULL, &osel_etimer_process, &event);
}

PROCESS(osel_etimer_process, "osel_etimer_process");
PROCESS_THREAD(osel_etimer_process, ev, data)
{
    PROCESS_BEGIN();

    while (1)
    {
        if (osel_etimer_htimer == NULL)
        {
            HAL_TIMER_SET_REL(MS_TO_TICK(osel_etimer_tick),
                              osel_etimer_tick_cb, NULL,
                              osel_etimer_htimer);
            DBG_ASSERT(osel_etimer_htimer != NULL __DBG_LINE);
        }
...
osel_bool_t osel_etimer_init(uint16_t ms)
{
#if OSEL_ETIMER_SOFTWARE
    DBG_ASSERT(ms >= 1 __DBG_LINE);
    osel_etimer_tick = ms;
    osel_pthread_create(os_task_tcb, &osel_etimer_process, NULL);
#endif
    return TRUE;
}
