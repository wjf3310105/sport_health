...
typedef struct _sqqueue_ctrl_t
{
    ...
    void        *(*del)        (struct _sqqueue_ctrl_t *const p_this);          //*< 队列首删除
    void        *(*revoke)     (struct _sqqueue_ctrl_t *const p_this);          //*< 队列尾删除
    uint16_t    (*get_len)     (const struct _sqqueue_ctrl_t *const p_this);    //*< 获取队列已经用成员个数
