...
URET uffs_TreeInit(uffs_Device *dev)
{
	...
	DBG_LOG(DBG_LEVEL_ORIGIN, "alloc tree nodes %d bytes.\r\n", size * num);
...
static u16 _GetBlockFromNode(u8 type, TreeNode *node)
{
	...
	DBG_LOG(DBG_LEVEL_ORIGIN, "unkown type, X-block\r\n");
...
static u16 _GetParentFromNode(u8 type, TreeNode *node)
{
	...
	DBG_LOG(DBG_LEVEL_ORIGIN, "unkown type, X-parent\r\n");
...
static u16 _GetSerialFromNode(u8 type, TreeNode *node)
{
	...
	DBG_LOG(DBG_LEVEL_ORIGIN, "unkown type, X-serial\r\n");
	return INVALID_UFFS_SERIAL;
}
#endif
								...
								struct BlockTypeStatSt *st)
{
	...
	if (node_alt != NULL) {
		...
		if (block_alt == INVALID_UFFS_SERIAL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "invalid block ?\r\n");
			return U_FAIL;
		}
	...
	if (type == UFFS_TYPE_DIR || type == UFFS_TYPE_FILE) {
        ...
        if (uffs_BadBlockAddByFlashResult(dev, block, ret) == UFFS_PENDING_BLK_NONE && UFFS_FLASH_HAVE_ERR(ret)) {
            DBG_LOG(DBG_LEVEL_ORIGIN, "I/O error ?\r\n");
            uffs_BufFreeClone(dev, buf);
            return U_FAIL;
        }
...
static URET _BuildTreeStepOne(uffs_Device *dev)
{
	...
	tree->erased_count = 0;
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "build tree step one\r\n");

//	printf("s:%d e:%d\n", dev->par.start, dev->par.end);
	for (block = dev->par.start; block <= dev->par.end; block++) {
		bc = uffs_BlockInfoGet(dev, block);
		if (bc == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "step one:fail to get block info\r\n");
			ret = U_FAIL;
			break;
		}
		node = (TreeNode *)uffs_PoolGet(pool);
		if (node == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "insufficient tree node!\r\n");
			ret = U_FAIL;
			break;
		}
		...
		if (uffs_FlashIsBadBlock(dev, block) == U_TRUE) {
            ...
            DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
			DBG_LOG(DBG_LEVEL_ORIGIN, "found bad block %d\r\n", block);
		}
	...
	if(ret == U_FAIL) 
		uffs_BlockInfoPut(dev, bc);
    char tmp[255];
    sprintf(tmp, "DIR %d, FILE %d, DATA %d\r\n", st.dir, st.file, st.data);
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN,"%s", tmp);
...
static URET _BuildTreeStepTwo(uffs_Device *dev)
{
	...
	TreeNode *node;
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "build tree step two\r\n");
...
static URET _BuildTreeStepThree(uffs_Device *dev)
{
	...
	pool = TPOOL(dev);
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "build tree step three\r\n");
...
URET uffs_BuildTree(uffs_Device *dev)
{
	...
	if (ret != U_SUCC) {
        DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
		DBG_LOG(DBG_LEVEL_ORIGIN, "build tree step one fail!\r\n");
		return ret;
	}
	...
	if (ret != U_SUCC) {
        DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
		DBG_LOG(DBG_LEVEL_ORIGIN, "build tree step two fail!\r\n");
		return ret;
	}
	...
	if (ret != U_SUCC) {
        DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
		DBG_LOG(DBG_LEVEL_ORIGIN, "build tree step three fail!\r\n");
		return ret;
	}
...
void uffs_BreakFromEntry(uffs_Device *dev, u8 type, TreeNode *node)
{
	...
	case UFFS_TYPE_DATA:
		...
		break;
	default:
		DBG_LOG(DBG_LEVEL_ORIGIN, "unknown type when break...\r\n");
		return;
	}
