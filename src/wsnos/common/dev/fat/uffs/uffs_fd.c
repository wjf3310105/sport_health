...
#define CHK_OBJ_LOCK(fd, obj, ret)	\
	do { \
		...
		if ( (fd >> FD_SIGNATURE_SHIFT) != _fd_signature ) { \
			...
			DBG_LOG(DBG_LEVEL_ORIGIN, "invalid fd: %d (sig: %d, expect: %d)\r\n", \
		...
		if ((obj) == NULL || \
			...
			uffs_set_error(-UEBADF); \
			DBG_LOG(DBG_LEVEL_ORIGIN, "invalid obj\r\n"); \
			uffs_GlobalFsLockUnlock(); \
			return (ret); \
		} \
	} while(0)
...
#define CHK_DIR_LOCK(dirp, ret)	\
	do { \
		...
		if ((dirp) == NULL || \
			...
			uffs_set_error(-UEBADF); \
			DBG_LOG(DBG_LEVEL_ORIGIN, "invalid dirp\r\n"); \
			uffs_GlobalFsLockUnlock(); \
			return (ret); \
		} \
	} while(0)
...
i32 uffs_seek(int fd, i32 offset, int origin)
{
	i32 ret;
	uffs_Object *obj;
...
u32 uffs_space_total(const char *mount_point)
{
	uffs_Device *dev = NULL;
	u32 ret = 0;
	...
	dev = uffs_GetDeviceFromMountPoint(mount_point);
	if (dev) {
		ret = (u32) uffs_GetDeviceTotal(dev);
		uffs_PutDevice(dev);
	}
	uffs_GlobalFsLockUnlock();
