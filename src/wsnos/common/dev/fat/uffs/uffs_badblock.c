...
static void process_pending_recover(uffs_Device *dev, uffs_PendingBlock *s)
{
	...
	if (bc == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "can't get bad block info\r\n");
		return;
	}
...
retry:
	...
	if (good == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "no free block to replace bad block!\r\n");
		uffs_BlockInfoPut(dev, bc);
		return;
	}
	...
	for (i = 0; i < dev->attr->pages_per_block; i++) {
		...
		if (page == UFFS_INVALID_PAGE) {
			...
			DBG_LOG(DBG_LEVEL_ORIGIN, "bad block recover (block %d) not finished!\r\n", bc->block);
			break;
		}
		tag = GET_TAG(bc, page);
		...
		if (UFFS_FLASH_HAVE_ERR(ret)) {
			if (UFFS_FLASH_IS_BAD_BLOCK(ret)) {
				...
				DBG_LOG(DBG_LEVEL_ORIGIN, "I/O error ? abort.\r\n");
		...
		if (buf->data_len > dev->com.pg_data_size) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "data length over flow!!!\r\n");
		...
		*newTag = *tag;
		...
		if (UFFS_FLASH_IS_BAD_BLOCK(ret)) {
			...
			DBG_LOG(DBG_LEVEL_ORIGIN, "A new bad block is discovered during bad block recover ...\r\n");
	...
	if (succRecov == U_TRUE) {
        ...
        switch(s->mark) {
            ...
            case UFFS_PENDING_BLK_CLEANUP:
                ...
                break;
            default:
                DBG_LOG(DBG_LEVEL_ORIGIN, "Unrecognized pending mark: %d\r\n", s->mark);
                break;
        }
	}
	else {
...
void uffs_BadBlockRecover(uffs_Device *dev)
{
	...
	while (dev->pending.count > 0) {
		...
		DBG_LOG(DBG_LEVEL_ORIGIN, "Process pending block %d - %s\r\n", 
...
void uffs_BadBlockAdd(uffs_Device *dev, int block, u8 mark)
{
	...
	for (i = 0; i < dev->pending.count; i++) {
		...
		if (s->block == block) {
			...
			if (s->mark < mark) { 	// RECOVER would overwrite REFRESH, MARKBAD would overwrite RECOVER/REFRESH
				s->mark = mark;
				DBG_LOG(DBG_LEVEL_ORIGIN, "Change pending block %d - %s\r\n",
	...
	s->block = block;
	s->mark = mark;
	DBG_LOG(DBG_LEVEL_ORIGIN, "Add pending block %d - %s\r\n",
...
URET uffs_BadBlockPendingRemove(uffs_Device *dev, int block)
{
	...
	if (i < dev->pending.count) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "pending block %d removed\r\n", dev->pending.list[i].block);
