...
DBG_THIS_MODULE("uffs_mem");
#define PFX "mem : "

#if CONFIG_USE_STATIC_MEMORY_ALLOCATOR > 0
static void * static_malloc(struct uffs_DeviceSt *dev, u32 size)
{
	...
	if (mem->buf_size - mem->pos < (u32)size) {
		...
		mem->pos += size;
        char tmp[255];
        sprintf(tmp,"%s0x%p: Allocated %ld, free %ld\r\n",PFX,p, size, mem->buf_size - mem->pos);
								  ...
								  void *pool, u32 size)
{
    ...
    DBG_LOG(DBG_LEVEL_ORIGIN,"System static memory: %d bytes\r\n", allocator->buf_size);
}
