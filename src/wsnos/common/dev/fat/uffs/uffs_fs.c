						   ...
						   int dir, const char *name, int name_len, int oflag)
{
	...
	if (obj->serial == INVALID_UFFS_SERIAL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "No free serial num!\r\n");
		obj->err = UENOMEM;
		goto ext_1;
	}
	...
	if (obj->node == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't find the node in the tree ?\r\n");
		obj->err = UEIOERR;
		goto ext_1;
	}
	...
	if (obj->type == UFFS_TYPE_DIR) {
		if (obj->name_len == 0) {
			if (dir != PARENT_OF_ROOT) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "Bad parent for root dir!\r\n");
				obj->err = UEINVAL;
			}
			else {
		...
		if (obj->name_len == 0 || name[obj->name_len - 1] == '/') {
			DBG_LOG(DBG_LEVEL_ORIGIN, "Bad file name.\r\n");
			obj->err = UEINVAL;
		}
	}
...
URET uffs_CloseObject(uffs_Object *obj)
{
	...
	if (obj->oflag & (UO_WRONLY|UO_RDWR|UO_APPEND|UO_CREATE|UO_TRUNC)) {
		...
		if (obj->node) {
			...
			if(buf == NULL) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "can't get file header\r\n");
				do_FlushObject(obj);
				uffs_ObjectDevUnLock(obj);
				goto ext;
			}
						  ...
						  u16 serial)
{
	...
	for (page_id = 0; page_id < dev->attr->pages_per_block; page_id++) {
		...
		if (buf == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "can't create a new page ?\r\n");
			break;
		}
		...
		if (ret != U_SUCC) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "write data fail!\r\n");
			break;
		}
		wroteSize += size;
							   ...
							   u32 blockOfs)
{
	...
	while (wroteSize < len) {
		...
		if ((obj->node->u.file.len % dev->com.pg_data_size) == 0 &&
			...
			if(buf == NULL) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "can create a new buf!\r\n");
				break;
			}
		}
		else {
			...
			if (buf == NULL) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "can't get buffer ?\r\n");
				break;
			}
		}
		...
		if (ret == U_FAIL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "write inter data fail!\r\n");
			break;
		}
...
static int do_WriteObject(uffs_Object *obj, const void *data, int len)
{
	...
	while (remain > 0) {
		...
		if (write_start > fnode->u.file.len) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "write point out of file ?\r\n");
			break;
		}
		...
		if (write_start == fnode->u.file.len && fdn > 0 &&
			...
			if (dev->tree.erased_count < dev->cfg.reserved_free_blocks) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "insufficient block in write obj, new block\r\n");
				break;
			}
			...
			if(dnode == NULL) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "can't find data node in tree ?\r\n");
...
int uffs_WriteObject(uffs_Object *obj, const void *data, int len)
{
	...
	if (obj->type == UFFS_TYPE_DIR) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't write to an dir object!\r\n");
		obj->err = UEACCES;
		return 0;
	}
...
int uffs_ReadObject(uffs_Object *obj, void *data, int len)
{
	...
	if (obj->type == UFFS_TYPE_DIR) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't read data from a dir object!\r\n");
		obj->err = UEBADF;
		return 0;
	}
	...
	while (remain > 0) {
		...
		if (read_start >= fnode->u.file.len) {
			//DBG_LOG(DBG_LEVEL_ORIGIN, "read point out of file ?");
			break;
		}
		...
		if (fdn == 0) {
			...
			dnode = uffs_TreeFindDataNode(dev, fnode->u.file.serial, fdn);
			if (dnode == NULL) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "can't get data node in entry!\r\n");
				obj->err = UEUNKNOWN_ERR;
				break;
			}
		}
		...
		if (buf == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "can't get buffer when read obj.\r\n");
			obj->err = UEIOERR;
			break;
		}
		...
		if (pageOfs >= buf->data_len) {
			//DBG_LOG(DBG_LEVEL_ORIGIN, "read data out of page range ?");
			uffs_BufPut(dev, buf);
			break;
		}
 ...
 */
i32 uffs_SeekObject(uffs_Object *obj, i32 offset, int origin)
{
	if (obj->type == UFFS_TYPE_DIR) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't seek a dir object!\r\n");
		obj->err = UEACCES;
	}
	else {
		uffs_ObjectDevLock(obj);
												...
												u16 fdn, u32 remain, RunOptionE run_opt)
{
	...
	if (buf == NULL) {
		obj->err = UENOMEM;
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't get buf\r\n");
		goto ext;
	}
	...
	if (flen < remain) {
		...
		if (run_opt == eREAL_RUN) {
			...
			if (do_WriteObject(obj, NULL, remain - flen) > 0) {	// fill '\0' ...
				DBG_LOG(DBG_LEVEL_ORIGIN, "Write object not finished. expect %d but only %d wrote.\r\n",
...
int _CheckObjBufRef(uffs_Object *obj)
{
		 ...
		 buf = uffs_BufFindFrom(dev, buf->next, obj->parent, obj->serial, UFFS_ALL_PAGES))
	{
		if (buf->ref_count > 0) {
			// oops ...
			DBG_LOG(DBG_LEVEL_ORIGIN, "someone still hold buf parent = %d, serial = %d, ref_count\r\n",
					   ...
					   int new_parent, const char *new_name, int name_len)
{
	...
	if (name_len > 0) {
		...
		if (buf == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "can't get buf when rename!\r\n");
			obj->err = UEIOERR;
			goto ext_1;
		}
...
URET uffs_RenameObject(const char *old_name, const char *new_name, int *err)
{
	...
	if (uffs_OpenObject(new_obj, new_name, oflag) == U_SUCC) {
		...
		DBG_LOG(DBG_LEVEL_ORIGIN, "new object already exist!\r\n");
		if (err)
			*err = UEEXIST;
		goto ext;
	}
	...
	if (uffs_ParseObject(new_obj, new_name) != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "parse new name fail !\r\n");
		if (err)
			*err = UENOENT;
		goto ext;
	}
	...
	if (new_obj->name_len == 0) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "invalid new name\r\n");
		if (err)
			*err = UEINVAL;
		goto ext;
	}
	...
	if (uffs_OpenObject(obj, old_name, oflag) != U_SUCC) {
		oflag |= UO_DIR;
		if (uffs_OpenObject(obj, old_name, oflag) != U_SUCC) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "Can't open old object !\r\n");
			if (err)
				*err = UEACCES;
			goto ext;
		}
