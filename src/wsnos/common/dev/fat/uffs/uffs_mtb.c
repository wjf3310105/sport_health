...
int uffs_Mount(const char *mount)
{
	...
	if (uffs_GetMountTableByMountPoint(mount, m_head) != NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN,	"'%s' already mounted\r\n", mount);
		return -1; // already mounted ?
	}
	
	mtb = uffs_GetMountTableByMountPoint(mount, m_free_head);
	if (mtb == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN,	"'%s' not registered\r\n", mount);
    ...
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "mount partiton: %d,%d\r\n",
		mtb->dev->par.start, mtb->dev->par.end);

	if (uffs_InitDevice(mtb->dev) != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "init device fail !\r\n");
		return -1;
	}
...
int uffs_UnMount(const char *mount)
{
	...
	if (mtb == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN,	"'%s' not mounted ?\r\n", mount);
		return -1;  // not mounted ?
	}

	if (uffs_GetMountTableByMountPoint(mount, m_free_head) != NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN,	"'%s' already unmounted ?\r\n", mount);
