...
typedef struct uffs_memAllocatorSt {
	...
	void * (*malloc)(struct uffs_DeviceSt *dev, u32 size); //!< allocate memory (for dynamic memory allocation)
	...
	u32 blockinfo_pool_size;			//!< block info cache buffers size
	u32 pagebuf_pool_size;				//!< page buffers size
	u32 tree_nodes_pool_size;			//!< tree nodes buffer size
	u32 spare_pool_size;				//!< spare buffer pool size

	uffs_Pool tree_pool;
	uffs_Pool spare_pool;

	u32 spare_data_size;				//!< spare size consumed by UFFS, 
...
void uffs_MemSetupStaticAllocator(uffs_MemAllocator *allocator, void *pool, u32 size);
