...
DBG_THIS_MODULE("uffs_debug");

#define MAX_DEBUG_MSG_LINE_LENGTH	128

static struct uffs_DebugMsgOutputSt * m_ops = NULL;
static u32 m_msg_level = UFFS_MSG_NORMAL;

static void uffs_vpru32f(const char *fmt, va_list args)
{
	char line_buf[MAX_DEBUG_MSG_LINE_LENGTH];

	if (m_ops && m_ops->output) {
#ifdef _COMPILER_CLIB_DO_NOT_HAVE_VSNPRINTF_
        DBG_LOG(DBG_LEVEL_ORIGIN,fmt);
		//vspru32f(line_buf, fmt, args);
#else
        DBG_LOG(DBG_LEVEL_ORIGIN,fmt);
		//vsnpru32f(line_buf, MAX_DEBUG_MSG_LINE_LENGTH, fmt, args);
#endif
...
URET uffs_InitDebugMessageOutput(struct uffs_DebugMsgOutputSt *ops, int msg_level)
{
	...
	else if (m_ops->vpru32f == NULL)
		m_ops->vpru32f = uffs_vpru32f;
...
void uffs_Perror(u32 level, const char *fmt, ...)
{
	va_list args;
	if (m_ops && level >= m_msg_level) {
		va_start(args, fmt);
		m_ops->vpru32f(fmt, args);
		va_end(args);
		m_ops->output(TENDSTR);
	}
}

void uffs_PerrorRaw(u32 level, const char *fmt, ...)
{
	va_list args;
	if (m_ops && level >= m_msg_level) {
		va_start(args, fmt);
...
void uffs_DebugMessage(u32 level, const char *prefix, const char *suffix, u32 line, const char *errFmt, ...) {};
void uffs_AssertCall(const char *file, u32 line, const char *msg, ...) {};
