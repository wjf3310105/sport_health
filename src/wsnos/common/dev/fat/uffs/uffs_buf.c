...
URET uffs_BufInit(uffs_Device *dev, int buf_max, int dirty_buf_max)
{
    ...
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "alloc %d bytes.\r\n", size);
	dev->buf.pool = pool;
...
//	DBG_LOG(DBG_LEVEL_ORIGIN, "Flush buffers with Block Recover, from %d to %d", 
//					bc->block, newBc->block);
	...
	for (i = 0; i < dev->attr->pages_per_block; i++) {
		...
		if (buf != NULL) {
			...
			if (buf == NULL) {  // no cached page buffer, use clone buffer.
				...
				if (buf == NULL) {
					DBG_LOG(DBG_LEVEL_ORIGIN, "Can't clone a new buf!\r\n");
					break;
				}
                ...
                if (uffs_BadBlockAddByFlashResult(dev, bc->block, flash_op_old) != UFFS_PENDING_BLK_NONE) {
					...
					buf->mark = UFFS_BUF_EMPTY;
					DBG_LOG(DBG_LEVEL_ORIGIN, "Read block %d page %d error (%d)\r\n", bc->block, page, flash_op_old);
			...
			if (buf->data_len > dev->com.pg_data_size) {
				DBG_LOG(DBG_LEVEL_ORIGIN, "data length over flow, truncated !\r\n");
	...
	if (UFFS_FLASH_IS_BAD_BLOCK(flash_op_new)) {
		...
		uffs_Perror(UFFS_MSG_NORMAL, "Retry block recover because of bad block...");
		...
		if (flash_op_new == UFFS_FLASH_IO_ERR) {
			...
			DBG_LOG(DBG_LEVEL_ORIGIN, "flash op error result: %d\r\n", flash_op_new);
		}
		uffs_BlockInfoPut(dev, newBc);
		uffs_Perror(UFFS_MSG_NORMAL, "Retry block recover because of flash op failure...");
	...
	if (flash_op_new == UFFS_FLASH_ECC_OK) {
		...
		uffs_BlockInfoPut(dev, newBc);
		uffs_Perror(UFFS_MSG_NORMAL, "Retry block recover because of refresh...");
		goto retry;
	}
#endif
	...
	if (succRecover == U_TRUE) {
		...
		case UFFS_TYPE_DATA:
			...
			break;
		default:
			DBG_LOG(DBG_LEVEL_ORIGIN, "UNKNOW TYPE\r\n");
			break;
		}
...
static URET _BufFlush_NewBlock(uffs_Device *dev, int slot)
{
	...
	if (node == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "no erased block!\r\n");
		goto ext;
	}
			   ...
			   UBOOL force_block_recover, int slot)
{
	...
	case UFFS_TYPE_DATA:
		...
		break;
	default:
		DBG_LOG(DBG_LEVEL_ORIGIN, "unknown type\r\n");
		return U_FAIL;
	}
...
URET uffs_BufUnLockGroup(struct uffs_DeviceSt *dev, int slot)
{
	...
	if (slot >= 0 && slot < dev->cfg.dirty_groups) {
		if (dev->buf.dirtyGroup[slot].lock > 0)
			...
			DBG_LOG(DBG_LEVEL_ORIGIN, "Try to unlock an unlocked group ?\r\n");
		}
		ret = U_SUCC;
	}	
	return ret;
					  ...
					  u8 type, u16 parent, u16 serial, u16 page_id)
{
	...
	if (buf) {
		if (buf->ref_count > 1) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "When create new buf, \
	...
	if (buf == NULL) {
		...
		buf = _FindFreeBuf(dev);
		if (buf == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "no free page buf!\r\n");
			return NULL;
		}
	}
						...
						u8 type, TreeNode *node, u16 page_id, int oflag)
{
	...
	bc = uffs_BlockInfoGet(dev, block);
	if (bc == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't get block info!\r\n");
		return NULL;
	}
	...
	if (page == UFFS_INVALID_PAGE) {
		uffs_BlockInfoPut(dev, bc);
		DBG_LOG(DBG_LEVEL_ORIGIN, "can't find right page ? block %d page_id %d\r\n", bc->block, page_id);
		return NULL;
	}
	page = uffs_FindBestPageInBlock(dev, bc, page);
	if (!uffs_Assert(page != UFFS_INVALID_PAGE, "got an invalid page?\n"))
	...
	if (pending_type == UFFS_PENDING_BLK_MARKBAD) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "can't load page from flash, non-recoverable bad block %d !\r\n", block);
		return NULL;
    }
    if (pending_type == UFFS_PENDING_BLK_NONE && UFFS_FLASH_HAVE_ERR(ret)) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "can't load page from flash, I/O error ?\r\n");
		return NULL;
	}
				   ...
				   uffs_Buf *buf, void *data, u32 ofs, u32 len)
{
	...
	if (slot < 0) {
		...
		if (slot < 0) {
			...
			if (slot < 0) {
				// still no free slot ??
				DBG_LOG(DBG_LEVEL_ORIGIN, "no free slot ?\r\n");
				return U_FAIL;
			}
		}
	}
