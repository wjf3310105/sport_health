...
UBOOL uffs_IsSrcNewerThanObj(int src, int obj)
{
	...
	case 2:
		return U_FALSE;
	default:
		DBG_LOG(DBG_LEVEL_ORIGIN,  "time stamp out of range !\r\n");
		break;
	}
						...
						uffs_BlockInfo *bc, uffs_FileInfo *fi)
{
	...
	if (buf == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "get buf fail.\r\n");
		return U_FAIL;
	}
 ...
 */
u32 uffs_GetDeviceUsed(uffs_Device *dev)
{
	return ((u32)(dev->par.end - dev->par.start + 1 -
			dev->tree.bad_count	- dev->tree.erased_count
			)) *
				((u32)dev->attr->page_data_size) *
					((u32)dev->attr->pages_per_block);
}

/** 
 * get partition free (bytes)
 */
u32 uffs_GetDeviceFree(uffs_Device *dev)
{
	return ((u32)dev->tree.erased_count) *
			((u32)dev->attr->page_data_size) *
				((u32)dev->attr->pages_per_block);
}
 ...
 */
u32 uffs_GetDeviceTotal(uffs_Device *dev)
{
	return ((u32)(dev->par.end - dev->par.start + 1)) *
				((u32)dev->attr->page_data_size) *
					((u32)dev->attr->pages_per_block);
