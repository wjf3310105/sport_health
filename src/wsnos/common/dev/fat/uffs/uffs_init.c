...
URET uffs_InitDevice(uffs_Device *dev)
{
    ...
    if (dev->attr->pages_per_block > UFFS_MAX_PAGES_PER_BLOCK) {
        uffs_Perror(UFFS_MSG_DEAD, "page_per_block should not exceed %d !\r\n", UFFS_MAX_PAGES_PER_BLOCK);
        return U_FAIL;
    }
	...
	if (dev->mem.init) {
		if (dev->mem.init(dev) != U_SUCC) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "Init memory allocator fail.\r\n");
			return U_FAIL;
		}
	}
	...
	if (uffs_FlashInterfaceInit(dev) != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't initialize flash interface !\r\n");
		goto fail;
	}
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "init page buf\r\n");
    ...
    DBG_LOG(DBG_LEVEL_ORIGIN,"%s", PFX);
	DBG_LOG(DBG_LEVEL_ORIGIN, "init block info cache\r\n");
	...
	if (ret != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "fail to init tree buffers\r\n");
		goto fail;
	}

	ret = uffs_BuildTree(dev);
	if (ret != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "fail to build tree\r\n");
		goto fail;
	}
...
URET uffs_ReleaseDevice(uffs_Device *dev)
{
	...
	if (ret != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN,  "fail to release block info.\r\n");
		goto ext;
	}

	ret = uffs_BufReleaseAll(dev);
	if (ret != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN,  "fail to release page buffers\r\n");
		goto ext;
	}

	ret = uffs_TreeRelease(dev);
	if (ret != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "fail to release tree buffers!\r\n");
		goto ext;
	}
	...
	if (ret != U_SUCC) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "fail to release memory allocator!\r\n");
	}
