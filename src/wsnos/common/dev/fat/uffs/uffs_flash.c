...
URET uffs_FlashInterfaceInit(uffs_Device *dev)
{
	...
	if (dev->ops->WritePage == NULL && dev->ops->WritePageWithLayout == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Flash driver must provide 'WritePage' or 'WritePageWithLayout' function!\r\n");
		goto ext;
	}

	if (dev->ops->ReadPage == NULL && dev->ops->ReadPageWithLayout == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Flash driver must provide 'ReadPage' or 'ReadPageWithLayout' function!\r\n");
		goto ext;
	}
	...
	if (dev->attr->layout_opt == UFFS_LAYOUT_UFFS) {
		...
		if (dev->ops->WritePageWithLayout == NULL || dev->ops->ReadPageWithLayout == NULL) {
			DBG_LOG(DBG_LEVEL_ORIGIN, "When using UFFS_LAYOUT_FLASH option, "
				"flash driver must provide 'WritePageWithLayout' and 'ReadPageWithLayout' function!\r\n");
			goto ext;
		}
	}
	else {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Invalid layout_opt: %d\r\n", dev->attr->layout_opt);
		goto ext;
	}

	if (dev->ops->EraseBlock == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Flash driver MUST implement 'EraseBlock()' function !\r\n");
		goto ext;
	}
	...
	if (dev->mem.spare_data_size > dev->attr->spare_size) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "NAND spare(%dB) can't hold UFFS spare data(%dB) !\r\n",
...
ext:
	...
	if (UFFS_FLASH_IS_BAD_BLOCK(ret)) {
		uffs_Perror(UFFS_MSG_NORMAL, "new bad block %d found while reading page %d tag\r\n", block, page);
	}
	else if (ret == UFFS_FLASH_ECC_OK) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "block %d page %d tag has bit flip and corrected by ECC\r\n", block, page);
	}
	else if (UFFS_FLASH_HAVE_ERR(ret)) {
		uffs_Perror(UFFS_MSG_NORMAL, "read block %d page %d tag failed, error = %d\r\n", block, page, ret);
	}
							   ...
							   uffs_Buf *buf, uffs_Tags *tag)
{
	...
	if (verify_buf) {
		...
		DBG_LOG(DBG_LEVEL_ORIGIN, "Insufficient buf, clone buf failed.\r\n");
	}
...
UBOOL uffs_FlashIsBadBlock(uffs_Device *dev, int block)
{
	...
	//DBG_LOG(DBG_LEVEL_ORIGIN, "Block %d is %s", block, ret ? "BAD" : "GOOD");
...
URET uffs_FlashCheckErasedBlock(uffs_Device *dev, int block)
{
	...
	if (spare == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't allocate spare buf.\r\n");
		goto ext;
	}
	...
	if (buf == NULL) {
		DBG_LOG(DBG_LEVEL_ORIGIN, "Can't clone buf.\r\n");
		goto ext;
	}
