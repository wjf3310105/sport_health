 ...
 * 2015/03/09  v0.0.2      gang.cheng    fix hard overflow bug
 */
...
static void hal_timer_set_overflow_interrupt(void)
{
    ...
    hal_time_t stime;
#if PF_CORE_POSIX
    stime.s.sw = global_timer_var.sw + 0;
    stime.s.hw = 0xFFFFu;
#else
    /**
     * 如果是真实硬件，这里模拟的溢出中断需要设置为0的比较中断，因为如果非0，有可能
     * 正好在关闭中断期间获取时间，这个时候需要人为的翻转软件时间，同时需要清除硬件
     * 时间，如果硬件比较值为0，就不需要这部分操作
     */
...
static void hal_timer_set_next_timer(bool_t result)
{
    ...
    else
    {
        ...
        /** 有可能now是处于翻转的时间之前是0xFFFF，设定之后是0x0001，发生了溢出，
            global_timer_var.sw并没有增加，所以这里断言去掉  */
...
void hal_timer_overflow_interrupt_handler(uint16_t line)
{
    ...
    HTIMER_HWCOUNT(now_hw);
    if (now_hw > 30)       //for debug
    {
        DBG_TIMER(0x23);
    }
    
    HTIMER_SW_INC(global_timer_var.sw);
